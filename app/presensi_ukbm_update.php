...
if ($ref != "") {
	...
	$idtingkat2	= $row_soal_siswa->idtingkat;
	$idkelas2	= $row_soal_siswa->idkelas;
	...
	$idpelajaran = $row_soal_siswa->idpelajaran;
								...
								<div style="overflow:auto; ">
									...
									<table class='table table-striped dataTable table-bordered' style="font-size:11px; ">
										...
										<tbody>
											...
											$sql=$select->list_presensi_ukbm_siswa('', '', $idtingkat2, $idkelas2, $nama, $all, 'SD');
											while ($row_siswa_detail=$sql->fetch(PDO::FETCH_OBJ)) {

												$i++;

												$tanggal = date("d-m-Y", strtotime($row_siswa_detail->tanggal));

												$sql2=$select->list_presensi_ukbm_detail($ref, $row_siswa_detail->replid, $idtingkat, $idkelas, $row_siswa_detail->tanggal);
											...
											<tr>
												...
												<td><?php echo $row_siswa_detail->nisn ?></td>
