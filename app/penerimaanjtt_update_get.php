...
<script>
	...
	{
		newWindow('kuitansijtt_multi.php?ref='+ref, 'CetakKuitansi','750','850','resizable=1,scrollbars=1,status=0,toolbar=0'		)
	 ...
	 function totals(jmldata){
		...
		for(i=0; i<=jmldata; i++){
			...
			if ( document.getElementById('bayar_'+i).value != isNaN || document.getElementById('bayar_'+i).value != 0) {				
				...
				bayar = document.getElementById('bayar_'+i).value.replace(/[^\d-.]/g,"");
...
$sqlstr = "SELECT nama FROM datapenerimaan WHERE replid = '$idpenerimaan' and departemen='$departemen'";
$sql=$dbpdo->prepare($sqlstr);
$sql->execute();
...
##get idjenis_bayar
$sqlstr = "select idjenis_bayar from penerimaanjtt where ref='$ref' limit 1";
$sqlj=$dbpdo->prepare($sqlstr);
$sqlj->execute();
$rowj = $sqlj->fetch(PDO::FETCH_OBJ);
$idjenis_bayar = $rowj->idjenis_bayar;
	...
	if($errmsg == "") {
		...
		for($v=0; $v<=$jmldata; $v++) {
			...
			if($bayar2 > 0) {
				...
				$cicilan = 1;
				$jml = 0;
				$sqlstr = "SELECT jumlah, idjurnal FROM penerimaanjtt WHERE idbesarjtt = '$idbesarjtt' and ref='$ref'";
				...
				/*if ($jml + $jcicilan <= $besarjtt) {*/
						...
						//// Simpan ke jurnal
						$success = UpdateJurnal($idtahunbuku, $tcicilan, $ketjurnal, $nokas, "", $petugas, "penerimaanjtt", $idjurnal);
						
						//// Simpan ke jurnaldetail
						if ($success) {
							$success = UpdateDetailJurnal($idjurnal, "D", $rekkas, $jcicilan);
						}
							
						if ($success) {
							$success = UpdateDetailJurnal($idjurnal, "K", $rekpiutang, $jcicilan);
						}*/
...
<tr height="150">
	...
	<table border="0" align="center" cellspacing="0" cellpadding="5">
		...
		<tr>
	        ...
	        <?php //if($nis != "") { ?>
	        	...
	        	<?php if($ref == "") { ?>
	        		<input type="text" name="tnis" id="tnis" size="25" value="<?php echo $nis ?>" onKeyPress="return focusNext('tbayar',event)" onblur="getpenerimaan()" style="font-weight: bold; height: 30px; font-size: 14px" >
	        	<?php } else { ?>
	        		<input type="text" name="tnis" id="tnis" size="25" value="<?php echo $nis ?>" readonly="" onKeyPress="return focusNext('tbayar',event)" style="font-weight: bold; height: 30px; font-size: 14px" >
	        	<?php } ?>
		...
		<tr>
	        ...
	        <td colspan="2">
	        <input type="text" name="tcicilan" id="tcicilan" readonly size="15" value="<?php echo $tanggal ?>"  style="background-color:#CCCC99">
		    </td>        
	    </tr>
	    ...
	    <tr>
	        ...
	        <td id="tbayar2">
		        ...
		        <?php if($ref == "") { ?>
			        ...
			        <input type="text" name="tbayar" id="tbayar" size="25" value="<?php echo $tbayar ?>" readonly="" onKeyPress="return focusNext('kcicilan',event)" onkeyup="formatangka('tbayar')" style="font-weight: bold; text-align: right; height: 30px; font-size: 14px" >
			        
		        <?php } else { ?>
		        	
		        	<input type="text" name="tbayar" id="tbayar" size="25" readonly="" value="<?php echo $tbayar ?>" style="font-weight: bold; text-align: right; height: 30px; font-size: 14px" >
		...
		<?php
			...
			if($ref != "") {
				...
				$sqlstr = " select '$nis' nis, b.besar, a.jumlah, b.cicilan, '' nama, '' kelas, '' tingkat, c.nama namaterima, b.idpenerimaan, a.replid id_pjtt, 1 nomor, ifnull(c.full,0) full, a.keterangan from penerimaanjtt a left join besarjtt b on a.idbesarjtt=b.replid left join datapenerimaan c on b.idpenerimaan=c.replid where b.nis='$nis' and a.ref='$ref' order by c.nourut ";
			...
			$terima = $terima + $besar;
				//if($sisa > 0) {
					...
					$total = $total + $jumlah; //$besar; //cicilan;
		...
		?>
					...
					<tr>
						...
						<td style="border: 1px solid #000" align="right"><?php echo formatnumericview($besar) ?></td>
						<td style="border: 1px solid #000" id="terima_<?php echo $z ?>"><input type="text" id="bayar_<?php echo $z ?>" readonly="" name="bayar_<?php echo $z ?>" value="<?php echo formatnumericview($jumlah) ?>" style="text-align: right" ></td>
		...
		<tr>
	        ...
	        <td colspan="3"><textarea id="kcicilan" name="kcicilan" rows="3" cols="30" onKeyPress="return focusNext('Simpan', event)"><?php echo $keterangan ?></textarea>	        
			</td>
	    </tr>
