...
<?php if($periodid == "Harian") { ?>
	...
	<table width="100%" border="1" cellspacing="0" style="font-family: sans-serif; font-size: 11px">
		...
		<?php
			...
			$sqlcount=$select_print->pnbp_dl_count_rpt($balai, $periodid, $fromdate, $todate);
			while ($datacnt = odbc_fetch_object($sqlcount)) {
				...
				$sql=$select_print->pnbp_dl_rpt($balai, $periodid, $dtecnt, '', $bulan, $tahun);	
			...
			?>
					<tr style="color: #000">
						...
						<td align="center"><?php echo number_format($data->jmlalat, 0, ",", ",") ?></td>
		...
		?>
				<tr style="font-weight: bold; background-color: #deecf3">
					...
					<td align="center"><?php echo number_format($subalat, 0, ",", ",") ?></td>
...
<?php if($periodid == "Bulanan") { ?>
	...
	<table width="100%" border="1" cellspacing="0" style="font-family: sans-serif; font-size: 11px">
		...
		<?php
			...
			$sqlcount=$select_print->pnbp_dl_countbulan_rpt($balai, $periodid, '', '', $bulan, $tahun);
			while ($datacnt = odbc_fetch_object($sqlcount)) {
				...
				$sql=$select_print->pnbp_dl_bulanan_rpt($balai, $periodid, $bulan, $tahun);	
				while ($data = odbc_fetch_object($sql)) {
...
<?php if($periodid == "Tahunan") { ?>
	...
	<table width="100%" border="1" cellspacing="0" style="font-family: sans-serif; font-size: 11px">
		...
		<?php
			...
			$sqlcount=$select_print->pnbp_dl_counttahun_rpt($balai, $periodid, '', '', $bulan, $tahun);
			while ($datacnt = odbc_fetch_object($sqlcount)) {
			
				$subalat = 0;
				$subtotal = 0;
					
				$sql=$select_print->pnbp_dl_tahunan_rpt($balai, $periodid, $bulan, $tahun);	
				while ($data = odbc_fetch_object($sql)) {
