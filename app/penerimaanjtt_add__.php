...
<script>
	...
	function cek_one(id, jmldata){
		//alert(jmldata);
		//var bayar=document.getElementById('bayar_'+id).value; 			
...
$idtahunbuku = "2";  //$_REQUEST['idtahunbuku'];
$bayar = numberreplace($_REQUEST['bayar']);
$tbayar = $_REQUEST['bayar'];

$dbpdo = DB::create();

$sqlstr = "SELECT a.nama, a.alamatsiswa, b.tingkat, c.kelas FROM siswa a left join kelas c on a.idkelas=c.replid left join tingkat b on c.idtingkat = b.replid WHERE a.nis = '$nis'";
		...
		for($v=0; $v<=$jmldata; $v++) {
			...
			if($bayar2 > 0) {
				...
				if ($jml + $jcicilan <= $besarjtt) {
						...
						if($rowscek == 0) {
							$sql = "INSERT INTO penerimaanjtt SET idbesarjtt='$idbesarjtt', idjurnal='$idjurnal', tanggal='$tcicilan', jumlah='$jcicilan', keterangan='$kcicilan', petugas='$petugas', ref='$ref' ";
						} else {
							$sql = "update penerimaanjtt set idbesarjtt='$idbesarjtt', idjurnal='$idjurnal', tanggal='$tcicilan', jumlah='$jcicilan', keterangan='$kcicilan', petugas='$petugas' where ref='$ref' and replid='$id_pjtt' ";
						}
...
<tr height="150">
	...
	<table border="0" align="center" cellspacing="0" cellpadding="5">
	    ...
	    <tr>
	        ...
	        <td id="tbayar2">
	        <input type="text" name="tbayar" id="tbayar" size="25" value="<?php echo $tbayar ?>" onblur="getpenerimaan_bayar()" onKeyPress="return focusNext('kcicilan',event)" onkeyup="formatangka('tbayar')" style="font-weight: bold; text-align: right; height: 30px; font-size: 14px" ></td>
		...
		?>
					...
					<tr>
						...
						<td style="border: 1px solid #000" id="terima_<?php echo $z ?>"><input type="text" id="bayar_<?php echo $z ?>" name="bayar_<?php echo $z ?>" value="<?php echo formatnumericview($cicilan) ?>" style="text-align: right" onkeyup="cek_one('<?php echo $z ?>', '<?php echo $jmldata; ?>')" ></td>
