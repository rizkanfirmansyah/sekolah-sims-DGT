...
$sql=$koneksi->query($sqlid);
$rowid=$sql->fetch_object();
...
while ($data2 = $results->fetch_object()) {	
	$nokas	=	$data2->namadp;	
	$jumlah	=	$data2->jumlah;
...
//echo $sql_multi2;
$res2=$koneksi->query($sql_multi2);
$jmldata1 = mysqli_num_rows($res2);
...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		/*$this->Cell(50,3,$namaid,0,1,'L',false);*/
		$this->Image('../img/logo.jpg', 40, 5, 10, 15, 'jpg', '');
		$this->Ln(10);
		...
		$this->Cell(2,2,':',0,0,'L',false);
		$this->Cell(50,2,substr($nama,1,28),0,1,'L',true);
		$this->Ln(2); 
	...
	function BasicTable($header,$data)
	{
		...
		{	
			...
			foreach($row as $col) {
				...
				if ($i==1) { $this->Cell(45,6,$col,1,0,"L"); }
				if ($i==2) { 
		...
		$this->Cell(45,2,'Penerima',0,1,'R',false);		
		$this->Ln(12);
		
		//$size = $size + $sizeadd;
		
		$this->Cell(25,2,'(____________)',0,0,'R',false);	
		$this->Cell(40,2,$getuser,0,1,'R',false);		
		$this->Ln(2);
...
$title='Bukti Setoran Tunas Unggul';
$pdf->SetTitle($title);	
$pdf->SetTitle($nis);	
$pdf->SetTitle($nama);
